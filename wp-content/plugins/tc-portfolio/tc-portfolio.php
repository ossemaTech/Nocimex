...
require_once TCPORTFOLIO_PLUGIN_DIR .'/lib/tc-portfolio-column.php';
 ...
 function tcportfolio_faq_enqueue_scripts() {
    //Plugin Main CSS File
     ...
     wp_enqueue_style('tcode-grid', TCPORTFOLIO_PLUGIN_URI.'/assets/css/tcode-grid.css');
...
// After Plugin Activation redirect
 ...
 //  page template
 function tcportfolio_get_custom_post_type_template($single_template) {
      global $post;
      $auto_height=tcp_option('height-auto','tc_single_page_style', true );
       $template='single-tcportfolio-center';
      if ($post->post_type == 'tcportfolio') {
           $single_template = dirname( __FILE__ ) . '/template/'.$template.'.php';
      }
      return $single_template;
 }
 add_filter( 'single_template', 'tcportfolio_get_custom_post_type_template' );
